...
def Predict_Disease(symptoms):
    ...
    for symptom in symptoms:
        Inp_vect[symptoms_dict[symptom]] = 1
...
def match_symptoms(user_symptoms):
    ...
    for symptom in user_symptoms:
        ...
        result = process.extractOne(symptom, symptoms_list)
        if result:  
            best_match = result[0]  
            score = result[1]      
            if score > 80:          
...
def home():
    if request.method == 'POST':
        ...
        sym = symp.split(',')
        user_symptoms = [s.strip() for s in sym]

        matched_symptoms = match_symptoms(user_symptoms)

        pred_disease = Predict_Disease(matched_symptoms)
        desc_pred, prec_pred, med_pred, diet_pred, work_Pred = Ai_Doc(pred_disease)

        predict_precautions = [i for i in prec_pred[0]]
        predict_medication = [i for i in med_pred[0]]
        predict_diet = [i for i in diet_pred[0]]

        return render_template('index.html', predicted_disease=pred_disease, description=desc_pred,
                               precautions=predict_precautions, medications=predict_medication,
                               diet=predict_diet, workout=work_Pred, symptoms=matched_symptoms)
